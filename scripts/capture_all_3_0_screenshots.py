"""
High-Resolution 3.0 Full-Page Screenshot Capture Suite for Jan-EPF AI
...
def capture_all():
    with sync_playwright() as p:
        ...
        print("[5/17] Capturing Full-Page I Need Money Hub (Para 68 Advance & Cheque OCR)...")
        ...
        print("[9/17] Capturing Full-Page Benchmarks Tab 1: 3-Way Evals Matrix...")
        page.goto(f"{BASE_URL}/benchmarks?key=damik2007", wait_until="networkidle")
        scroll_full_page(page)
        page.screenshot(path=f"{OUTPUT_DIR}/09_benchmarks_evals_matrix.png", full_page=True)

        print("[10/17] Capturing Full-Page Benchmarks Tab 2: 1,000-Run Latency Runner...")
        latency_tab = page.locator("text=1,000-Run Latency").first
        if latency_tab.is_visible():
            latency_tab.click()
            time.sleep(1.0)
        scroll_full_page(page)
        page.screenshot(path=f"{OUTPUT_DIR}/10_benchmarks_latency_runner.png", full_page=True)

        print("[11/17] Capturing Full-Page Benchmarks Tab 3: Trace & Token Receipts...")
        trace_tab = page.locator("text=Raw Trace").first
        if trace_tab.is_visible():
            trace_tab.click()
            time.sleep(1.0)
        scroll_full_page(page)
        page.screenshot(path=f"{OUTPUT_DIR}/11_benchmarks_trace_console.png", full_page=True)

        print("[12/17] Capturing Full-Page Benchmarks Tab 4: National Exchequer ROI Calculator...")
        roi_tab = page.locator("text=National Exchequer ROI").first
        if roi_tab.is_visible():
            roi_tab.click()
            time.sleep(1.0)
        scroll_full_page(page)
        page.screenshot(path=f"{OUTPUT_DIR}/12_benchmarks_roi_calculator.png", full_page=True)

        print("[13/17] Capturing Full-Page Benchmarks Tab 5: Security & SRE Audit...")
        sec_tab = page.locator("text=Security & SRE Audit").first
        if sec_tab.is_visible():
            sec_tab.click()
            time.sleep(1.0)
        scroll_full_page(page)
        page.screenshot(path=f"{OUTPUT_DIR}/13_benchmarks_security_audit.png", full_page=True)

        print("[14/17] Capturing Full-Page Architecture Tab 1: 1.98M Grievance Root Causes...")
        page.goto(f"{BASE_URL}/architecture?key=damik2007", wait_until="networkidle")
        scroll_full_page(page)
        page.screenshot(path=f"{OUTPUT_DIR}/14_architecture_grievances.png", full_page=True)

        print("[15/17] Capturing Full-Page Architecture Tab 2: DPDP Act 2023 Blueprint...")
        dpdp_tab = page.locator("text=DPDP Act 2023").first
        if dpdp_tab.is_visible():
            dpdp_tab.click()
            time.sleep(1.0)
        scroll_full_page(page)
        page.screenshot(path=f"{OUTPUT_DIR}/15_architecture_dpdp_blueprint.png", full_page=True)

        print("[16/17] Capturing Full-Page Architecture Tab 3: Demographic Personas...")
        cohort_tab = page.locator("text=Demographic Personas").first
        if cohort_tab.is_visible():
            cohort_tab.click()
            time.sleep(1.0)
        scroll_full_page(page)
        page.screenshot(path=f"{OUTPUT_DIR}/16_architecture_demographics.png", full_page=True)

        print("[17/17] Capturing Full-Page Architecture Tab 7: Tools & Tech Stack Matrix...")
        tech_tab = page.locator("text=Tools & Tech Stack").first
        if tech_tab.is_visible():
            tech_tab.click()
            time.sleep(1.0)
        scroll_full_page(page)
        page.screenshot(path=f"{OUTPUT_DIR}/17_architecture_tech_stack.png", full_page=True)

        browser.close()
        print(f"\n🎉 Successfully captured all 17 FULL-PAGE screenshots into {OUTPUT_DIR}/")
