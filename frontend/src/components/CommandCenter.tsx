...
export function CommandCenter({ isOpen, onClose, onOpenChaosSimulator }: CommandCenterProps) {
  ...
  const actionsCategory = `⚡ ${activeName}'s Account Actions`;
  ...
  const uanNumber = activeCitizen?.uan || "000000000000";

  // Build 100% account-specific items tailored to activeCitizen without cross-account pollution
  const items = [
    // ⚡ 1. Deep, personalized priority actions for ACTIVE citizen
    {
      id: "action-advance",
      category: actionsCategory,
      title: `💰 Apply for Emergency Advance (Form 31) - Up to ₹${maxSanction}`,
      subtitle: `Instant pre-flight check & direct DBT transfer for ${firstName} (${establishmentName})`,
      icon: Wallet,
      action: () => {
        router.push("/money");
        onClose();
    ...
    },
    {
      ...
      subtitle: `1-Click Multi-Job Consolidation & Section 192A 0% TDS Tax Shield for ${firstName}`,
      icon: Briefcase,
      action: () => {
        router.push("/career");
        onClose();
    ...
    },
    {
      ...
      subtitle: `Visual Passbook Triple-Split & 8.25% Sovereign Retirement Forecast for ${firstName}`,
      icon: PiggyBank,
      action: () => {
        router.push("/savings");
        onClose();
    ...
    },
    {
      ...
      subtitle: `Levenshtein Fuzzy Match, NPCI Penny Drop & Joint Declaration for ${firstName}`,
      icon: Wrench,
      action: () => {
        router.push("/fix");
        onClose();
    ...
    },
    {
      id: "action-enom",
      category: actionsCategory,
      title: `📋 Update e-Nomination & Claim ₹7 Lakh EDLI Free Insurance`,
      subtitle: `Statutory Life Cover nomination registration for ${firstName}'s family`,
      icon: ShieldCheck,
      action: () => {
        router.push("/fix");
        onClose();
    ...
    },
    {
      ...
      category: actionsCategory,
      title: `📄 Download Official Passbook Statement PDF (UAN: ${uanNumber})`,
      subtitle: `Generate cryptographically signed PF passbook receipt for ${firstName}`,
      icon: Download,
      action: () => {
        router.push("/savings");
        onClose();
    ...
    },
    {
      id: "action-chaos",
      category: actionsCategory,
      title: `⚡ Run Zero-Rejection Chaos Test on ${firstName}'s Record`,
    ...
    // 🏛️ 2. Human Life-Event Hubs Navigation
    {
      id: "hub-money",
      category: "🏛️ Life-Event Topic Hubs",
      title: "I Need Money (Advance Withdrawal)",
      subtitle: "Statutory Form 31 Advance with Instant In-Browser Pre-Flight Check",
      icon: Wallet,
      action: () => {
        router.push("/money");
        onClose();
    ...
    },
    {
      ...
      subtitle: "1-Click Multi-Job Consolidation & ECR Date of Exit Auto-Deduction",
      icon: Briefcase,
      action: () => {
        router.push("/career");
        onClose();
    ...
    },
    {
      ...
      subtitle: "Visual Passbook Triple-Split & 30-Year Retirement Wealth Forecast",
      icon: PiggyBank,
      action: () => {
        router.push("/savings");
        onClose();
    ...
    },
    {
      ...
      subtitle: "Levenshtein Fuzzy Match, Sub-200ms NPCI Penny Drop & Digital Joint Declaration",
      icon: Wrench,
      action: () => {
        router.push("/fix");
        onClose();
    ...
    {
      id: "hub-benchmarks",
      category: "📊 Proof Assets & Evals",
      title: "⚡ 1,000-Run Latency Runner & Evals Matrix",
      subtitle: "Live CPU microsecond benchmarks (<0.05ms) & OpenAI 3-part proof standard",
      icon: Award,
      action: () => {
    ...
    {
      id: "hub-architecture",
      category: "🏛️ DPI Research Lab",
      title: "🏛️ Architecture & 18-Tool Tech Matrix",
      subtitle: "1.98M Grievance Analysis, Demographic Personas & 18-Technology Toolchain",
      icon: Landmark,
      action: () => {
    ...
    // ⚙️ 3. System & Accessibility Toggles
    {
