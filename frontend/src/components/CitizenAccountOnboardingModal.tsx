    ...
    switch (langCode) {
      ...
      default:
        return {
          ...
          step1Badge: isGurmeet ? "Step 1 of 4 • Senior Pension Foundation" : "Step 1 of 4 • Account Foundation",
          step1Title: isGurmeet ? `👤 Welcome ${firstName}! Your EPS-95 Pension & Identity` : `👤 Welcome ${firstName}! Your EPF Identity & Balance Split`,
          step1Sub: isGurmeet ? `Universal Account Number (UAN): ${uan} • PPO #PBASR00012345` : `Universal Account Number (UAN): ${uan}`,
          step1P1: isGurmeet ? `Pension Disbursing Agency: Punjab National Bank (100% Verified Active Account).` : `Active Establishment: ${activeEmployer} with 100% verified KYC.`,
          step1P2: isGurmeet ? `Monthly EPS-95 Pension: ₹4,250 / month direct benefit transfer on the 1st of every month.` : `Triple-Split Passbook: ₹${empShare} (Employee 12%) + ₹${emprShare} (Employer 3.67%) + ₹${epsShare} (EPS-95 Pension).`,
          step1P3: isGurmeet ? `Digital Life Certificate (DLC): Verified & Active (Jeevan Pramaan Face RD valid until Nov 2026).` : `8.25% FY Annual Compounding: Interest calculates monthly and credits directly into your sovereign ledger.`,
          step1P4: isGurmeet ? `Settled EPF Corpus: ₹5,80,000 retirement provident corpus fully disbursed at superannuation.` : `Zero Employer Friction: Your UAN remains portable throughout your entire working career across India.`,
          step1BoxLabel: isGurmeet ? "Monthly EPS-95 Pension" : "Total Sovereign Corpus",
          step1BoxVal: isGurmeet ? "₹4,250 / mo" : `₹${totalBalance}`,
          step1BoxSub: isGurmeet ? "Direct Benefit Transfer • Punjab National Bank" : "3 Split Funds Active • 8.25% Interest Compounding",
  ...
  const STEPS = useMemo(() => [
    {
      ...
      highlightBox: {
        ...
        val: localizedText.step1BoxVal || (totalBalance.startsWith("₹") ? totalBalance : `₹${totalBalance}`),
